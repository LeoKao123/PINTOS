...
void fsutil_extract(char** argv UNUSED) {
  ...
  for (;;) {
    ...
    block_cache_read(src, sector++, header);
    ...
    if (type == USTAR_EOF) {
      ...
      while (size > 0) {
        ...
        block_cache_read(src, sector++, data);
  ...
  block_cache_write(src, 0, header);
  block_cache_write(src, 1, header);
...
void fsutil_append(char** argv) {
  ...
  block_cache_write(dst, sector++, buffer);
  ...
  block_cache_write(dst, sector, buffer);
  block_cache_write(dst, sector + 1, buffer);
